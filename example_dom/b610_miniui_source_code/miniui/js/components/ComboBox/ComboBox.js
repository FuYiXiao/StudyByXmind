...
mini.extend(mini.ComboBox, mini.PopupEdit,{
	...
	select: function (item) {
		...
		if (item) {
			...
			this.__OnItemClick();
            //解决当没有点击过下拉按钮时，做select操作后，无法设置选中样式 潘正锋2013-06-28
	...
	setUrl: function (url) {
		...
		this._listbox.setUrl(this.parseUrl(url));
    ...
    /**
     * 设置是否显示列表头部，史涛 2015-4-15
     * @type boolean
     * @default false
     */
    setShowColumns: function (value) {
        if (this._listbox)
            this._listbox.setShowColumns(value);
	...
	setValue: function (value,valid) {
        ...
        if (this.valueFromSelect) {
         ...
         //将if判断移至这里，让每次调用此方法都执行上面的代码,修复当刚进入页面就执行select方法后，下拉列表中的选中项没有被选中的问题潘正锋 2013-06-06
	...
	__OnItemClick: function (e) {
		...
		var value = this.getValue();
		this.setValue(vts[0]);
        //setValue方法中已经有setText方法了 潘正锋 2013-06-27
		//this.setText(vts[1]);
		if (e) {
	...
	getAttrs: function (el) {
        ...
        // 添加showColumns属性，增加可以显示列表头部的功能，史涛 2015-4-15
		mini._ParseBool(el, attrs,
		["multiSelect", "showNullItem", "valueFromSelect","showColumns"
		]
		);
