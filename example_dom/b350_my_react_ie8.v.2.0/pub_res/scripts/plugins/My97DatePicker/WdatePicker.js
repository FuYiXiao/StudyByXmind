...
(function() {
	...
	if (!U[M].docMD) {
		E(U[M], "onmousedown", D);
        //解决当前页面刷新或重新打开后无法正常监听到mousedown，日期层不会关闭的问题 赵美丹 2015-02-05
		//U[M].docMD = true
	}
	...
	function B() {
		...
		for ( var $ in obj)
			U.$dp[$] = obj[$];
		$dp = U.$dp;
		//解决IE9下iframe页面刷新后日期选择层无法正常弹出问题 赵美丹 2013-08-09
		$dp.dd=U[M].getElementById("_my97DP");
	}
	function E(A, $, _) {
		if (R)
			A.attachEvent($, _);
	...
	function T(N, F) {
		...
		if (F) {
			...
			if (J && N.el.nodeType == 1 && N.el["My97Mark"] === undefined) {
				var A, D;
				//增加J.type == "Focus"的判断，解决onFocus调用时监听不正常的问题  赵美丹 2012-10-14
				if (J.type == "focus" || J.type == "Focus") {
					A = "onclick";
					D = "onfocus"
				} else {
					A = "onfocus";
				...
				//当N.el[D]不存在时，主动触发事件D，解决通过jquery绑定事件方式定义时无法正常触发D事件的问题  赵美丹 2012-10-14
				var handler = N.el[D];
				if(!handler){
					handler = function(){
						if(R){
							...
							N.el.dispatchEvent(D.replace(/on/, ""));
						}
					}
				}
				E(N.el, A, handler);
			}
			//调整I(N)与if (J && N.el.nodeType == 1 && N.el["My97Mark"] === undefined)的执行顺序，确保每个输入框的事件绑定。
			//否则onfocus监听时，当选择日期后再点击输入框不再弹出日期控件 赵美丹 2012-11-14
			I(N);
		}
		function L() {
	...
	function I(G, _) {
		...
		function E(F, E) {
            ...
            var D,_ = $.$langList,C = $.$skinList;
            try {
                D = F.lastChild.contentWindow[M];
            } catch (ex) {
                crossDomain = true;
                F.removeChild(F.lastChild);
				var iframe = U[M].createElement("iframe");
                ...
                iframe.src ="javascript:(function(){var d=document;d.open();d.domain='" + U[M].domain + "';d.close();})()";
            ...
            function loadIframe() {
                ...
                if (crossDomain)
                    G.splice(1,0,"document.domain=\"" + U[M].domain + "\";");
