...
mini.extend(mini.ListBox, mini.ListControl,{
	...
	doLayout: function () {
		...
		if (isIE) {
			...
			if (this._viewEl.offsetHeight >= this._viewEl.scrollHeight) {
			    ...
			    //listbox在ie6下窗口大小改变后出现滚动条的问题。潘正锋 2013-06-19
				//为了解决上面问题，删除下面一行代码后又有新的问题，有滚动条的情况下，会错位，又取消了删除,无法复现上面的问题  2015-03-08
	...
	getAttrs: function (el) {
		...
		["nullItemText", "ondrawcell","emptyText"
		]
		);
		mini._ParseBool(el, attrs,
		["showCheckBox", "showAllCheckBox", "showNullItem", "showColumns","showEmpty"
		]
		);
