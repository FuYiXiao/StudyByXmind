...
mini.extend(mini.Tree, mini.Control, {
    ...
    destroy: function (removeEl) {
        ...
        //前面没有在document上绑定任何事件，删除下面代码 pzf 2015-06
    ...
    _initEvents: function () {
        mini._BindEvents(function () {
            ...
            // 新增mouseover事件的监听 潘正锋 2014-02-18
    ...
    _createNodeTitle: function (node, useEdit, sb) {
        ...
        /**下面代码貌似没有用到 潘正锋 */
        //var subNodes = this._getViewChildNodes(node);
    ...
    _createNode: function (node, sb) {
        ...
        // the code below is in createNodeTiele before now move it to here pzf 2014-07
        ...
        //拖动非叶子节点下的节点，且该非叶子节点下只有一个子节点，子节点被拖走后，遗留下children属性，导致会生成一个空行 潘正锋 2013-07-05
    ...
    doLayout: function () {
        ...
        //本来这里肯定没事的  但是一旦放在表格里快速翻页就报为空了，还是加个判断吧 潘正锋 2013-07-20
        if (this.el) {
    ...
    __OnEditInputKeyDown: function (e) {
        if (e.keyCode == 13) {
            ...
            //调整顺序:先事件后cancel,解决_editingNode被清空的问题 赵美丹 2015/12/22
    ...
    _doCheckNode: function (node, checked, checkRecursive) {
        ...
        ckNodes.push(node);
        //checkRecursive,checkOpposite,autoCheckParent 同时配置时，确实不好处理，checkRecursive和autoCheckParent同时存在没什么问题
        // 但是一旦配置了checkOpposite，checkRecursive，autoCheckParent就无效了 pzf 2015-02
        if (checkRecursive) {
        ...
        //新增的功能 父子节点互斥选择，选择了父就不选子，选择了子就不选择父 pzf 2015-02
        if(this.checkOpposite){
            //到这里为止，前两个if中放进去的node都无效，因为前两项配置都会失效
            ckNodes = [];

            ckNodes.push(node);
            //遍历下面的所有节点,这里不用处理!checked的情况，因为下面的节点肯定是unchecked
            if(node.checked) {
                this.cascadeChild(node, function (cnode) {
                    cnode.checked = false;
                    cnode._indeterminate = false;
            ...
            for (var i = 0, l = ans.length; i < l; i++) {
                var pnode = ans[i];
                //这里不用处理!checked的情况，因为下面的节点肯定是unchecked
                if(node.checked) {
                    pnode.checked = false;
                    pnode._indeterminate = false;
                }
    ...
    /***
     * xdj 2015-07 获得过滤后的节点
     * viewNodes中存放过滤之后的节点，如果viewNodes不存在那么说明没有过滤
     * @param haveParent
     * @returns {Array}
     */
    getCheckedNodes: function (haveParent) {
        ...
        var filter ={};
        var views = [];
        var viewNodes = this._viewNodes ? this._viewNodes : null;

            this.cascadeChild(this.root, function (node) {
                if ((viewNodes===null && node.checked == true)||(node.checked == true && viewNodes.hasOwnProperty(node._id) )) {
                    nodes.push(node);
                    if (haveParent) {
                        var ans = this.getAncestors(node);
                        for (var i = 0, l = ans.length; i < l; i++) {
    ...
    __OnClick: function (e) {
        ...
        var node = this._getNodeByEvent(e);
        if (node && node.enabled !== false) {
            //节点编辑时,点击输入框不影响节点的状态变化 赵美丹 2015-12-23
            if(mini.hasClass(e.target, "mini-tree-editinput")) return;
            ...
            if ((mini.findParent(e.target, this._eciconCls) || allow)
                ...
                //checked值相反，这里及时的修正 pzf 2014-12
    ...
    __OnMouseDown: function (e) {
        if (!this.enabled) return;
        //beginedit之后鼠标点击输入框，输入框会消失，无法正常编辑 pzf 2014-12
    ...
    focusNode: function (node, view) {
        ...
        var me = this;
        //在选中某个节点后，鼠标在另一个节点上晃动，会导致该节点也会被选中 ie8  pzf 2014-11
      //  setTimeout(function () {
            doFocus.call(me);
       // }, 1);
    },
    ...
    //为了修改treegrid拖拽而新增加的方法 判断是否可以拖拽 pzf
    _allowDrag:function(e){
        return true;
    },
    getAttrs: function (el) {
        var attrs = mini.Tree.superclass.getAttrs.call(this, el);
        // 新增nodemouseover "imgPath", "imgField"潘正锋 2014-02-18
        // autoLoad 潘正锋 2014-03
        ...
        mini._ParseBool(el, attrs,
            ["allowSelect", "showCheckBox", "showExpandButtons", "showTreeIcon", "showTreeLines", "checkRecursive",
                ...
                "autoCheckParent", "loadOnExpand", "expandOnNodeClick", "autoEscape","autoLoad","checkOpposite"
            ]
        );
        if (attrs.ajaxOption) {
