...
mini = {
    ...
    getbyName: function (name, parentNode) {
        ...
        //特殊处理，组件内部定义了getbyName的情况下以内部方法为准。menu.js里面有这个方法
...
mini._ParseColumns = function (el) {
    ...
    for (var i = 0, l = cs.length; i < l; i++) {
        ...
        //digit应经从api中移除了，被decimalPlaces替代 pzf 2015-02
        ...
        /**修改autoEscape默认为true潘正锋 2013-07-20*/
...
mini._doOpen = function (options) {
    ...
    //增加closeAction的配置,只有在没配置时才默认是destroy pzf 2015-03-08
...
mini.open = function (options) {
        ...
        if (currentWindow)
            ...
            if (basepath) {
                ...
                if (firstChar == "/") {
                    ...
                    //解决?后面内容中有/出现，base64加密后什么字符都会出现，所以先要把问号后面的截掉 潘正锋 2013-10-29
    ...
    var ps = [];
    //当父页面跨域时，这个方法会报错 潘正锋 2014-06-05
    function getParents(me) {
        try {
            if (me.mini)
                ps.push(me);
...
(function () {
    ...
    mini.getAttr = function (el, name) {
        ...
        // ie7 ie8兼容模式下onload属性无法获取 pzf 2015-06
        if(!v && name == "onload"){
...
mini.isWindowDisplay = function () {
    try {
        ...
        if (isIFrame) {
            ...
            //如果没有找到window对象匹配的Iframe则代表该窗口尚不可用。也就是没有显示完成
            if (!iframe)
                return false;
...
//将对象嵌套转化为命名空间，例如将vo:{age:1}转化为vo.age:1
//测试数据 {d: {g: 1,h:{i:1,j:2}}}，输出{ 'd.g': 1, 'd.h.i': 1, 'd.h.j': 2 }
mini.objToLine = function(obj, namepase) {

    var arr = {};
    if(obj == null) return arr;
    a(obj, namepase);
    function a(obj, namepase) {
        for (var key in obj) {
            var item = obj[key];
            if (item === null || item === undefined) {
                if (namepase)
                    arr[namepase + '.' + key] = obj[key];
                else
                    arr[key] = obj[key];
            }
            else if (Object.prototype.toString.call(obj[key]) == "[object Object]") {
                if (namepase)
                    arguments.callee(item, namepase + '.' + key);
                else
                    arguments.callee(item, key);
